...
export class DynamicFieldService {
  ...
  static async loadFields() {
    try {
      // 1. Try loading from MERN API (MongoDB)
      try {
        const res = await ApiClient.get('/fields');
        const list = res?.data || res?.fields || (Array.isArray(res) ? res : []);
        if (Array.isArray(list) && list.length > 0) {
          this.fields = list;
          this.buildFieldMap();
          console.log(`Loaded ${this.fields.length} dynamic study fields from MERN API`);
          return this.fields;
        }
      } catch (err) {
        console.warn('ApiClient /fields fetch failed, trying Supabase fallback:', err.message);
      }
      ...
      if (error || !Array.isArray(data) || data.length === 0) {
        console.warn('No dynamic study fields found in database, using fallback options.');
        this.initializeFallbackFields();
        return this.fields;
      }
      ...
      console.warn('Failed to fetch study fields, using fallbacks:', err.message);
      this.initializeFallbackFields();
      return this.fields;
    }
  }
