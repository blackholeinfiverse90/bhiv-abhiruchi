  ...
  fields: [
    ...
    },
    {
      ...
      options: [
        ...
        { value: "health_medicine", label: "Health & Medical Sciences" },
    ...
    },
    {
      ...
      options: [
        ...
        { value: "vedic_knowledge", label: "Vedic Knowledge & Philosophy" },
        { value: "current_affairs", label: "Current Affairs & General Knowledge" }
      ]
    },
    {
      id: "current_skills",
...
export class FormConfigService {
  ...
  static async getActiveFormConfig() {
    try {
      // 1. Try MERN API (MongoDB)
      try {
        const res = await ApiClient.get('/forms');
        if (res && res.data) {
          const mernConfig = res.data;
          mernConfig.fields = Array.isArray(mernConfig.fields) ? mernConfig.fields : [];
          const hasField = (id) => mernConfig.fields.some(f => f.id === id);
          if (!hasField('grade')) {
            mernConfig.fields.push({
              id: 'grade',
              type: FIELD_TYPES.SELECT,
              label: 'Grade',
              required: true,
        ...
        console.warn('ApiClient /forms fetch failed, trying Supabase fallback:', err.message);
      }
