...
export default function StudentAnalytics() {
  ...
  const loadStudentData = async () => {
    try {
      ...
      // 1. Try MERN Express Backend
      try {
        const res = await ApiClient.get('/students/attempts');
        const attemptsList = res?.data || res?.attempts || (Array.isArray(res) ? res : []);
        if (Array.isArray(attemptsList) && attemptsList.length > 0) {
          list = attemptsList;
        }
      } catch (err) {
        console.warn('ApiClient /students/attempts fetch failed, falling back to Supabase:', err.message);
      }
      ...
      if (list.length === 0) {
        ...
        if (attemptsData) list = attemptsData;
      }
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    <div className="text-white font-medium flex items-center justify-end gap-2">
                      <span>{(attempt.percentage ?? 0).toFixed(1)}%</span>
...
function StudentDetailsModal({ student, onClose, getSeedLevel }) {
  ...
  const loadStudentResponses = async () => {
    try {
      ...
      // Check if responses exist on student attempt object directly
      if (Array.isArray(student.evaluated_responses) && student.evaluated_responses.length > 0) {
        setResponses(student.evaluated_responses);
        return;
      }
  ...
  const timeSeconds = Number(student.time_taken_seconds || student.time_taken) || 120;
  const displayMinutes = Math.max(1, Math.floor(timeSeconds / 60));
  const displaySeconds = timeSeconds % 60;
        ...
        >
          ...
          <div className="grid gap-4 md:grid-cols-4 mb-6">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-green-500/20 to-green-600/10 backdrop-blur-sm border border-green-500/30 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white">{responses.length || (student.evaluated_responses?.length ?? 10)}</div>
            </div>
          </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {responses.map((response, index) => (
                  <div key={response.id || response.question_id || `resp_${index}`} className="p-6 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 hover:border-orange-500/50 transition-all duration-300 shadow-lg hover:shadow-xl">
