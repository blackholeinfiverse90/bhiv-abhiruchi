...
export default function QuestionBankManager() {
  ...
  async function loadData() {
    try {
      ...
      // 1. Try loading questions from MERN API (MongoDB)
      try {
        ...
        console.warn('API /questions fetch failed, falling back to Supabase:', err.message);
      }

      // Fallback to Supabase for questions if MERN API returned empty
      if (questionsList.length === 0) {
        ...
        if (questionsData) questionsList = questionsData;
      }
      setQuestions(questionsList);

      // 2. Try loading study fields from MERN API (MongoDB)
      try {
        ...
        console.warn('API /fields fetch failed, falling back to Supabase:', err.message);
      }

      // Fallback to Supabase for study fields
      if (fieldsList.length === 0) {
        ...
        if (fieldsData) fieldsList = fieldsData;
      }
      setStudyFields(fieldsList);
